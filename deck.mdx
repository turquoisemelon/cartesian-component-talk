...
# Cartesian Component:
## Automating Component Variation Rendering
...
<Notes>
  ...
  A single source of truth for all of the product teams. It's fundamental purpose is to facilitate the work of different teams by preventing them to design and build same things.
</Notes>
...
<Notes>
  Design language: the overall visual design of a digital product.
  Defines characteristics such as typography, colors, icons, spacing and so on. Maintained as Design Tokens in code. 
  ...
  Component library: a set of JavaScript components that are version controlled and can be composed to build applications and products.
  Developer Sandbox: a tool for DS developers. Used to develop components in isolation, document use cases and write tests  without having to run the entire application.
  Documentation: guidelines on how to consume the DS, design language, design and dev considerations and detailed documentation for each component.
</Notes>
---
...
<Notes>
  Last words before wrapping up. Benefits of Cartesian Component approach:
  Improves development process by helping to visualize how every component looks in its different variations
  Provided Instant feedback. Shorten the cycle between code tweaking to visual output
  being able to see everything at the same time, whether things are broken or not 
  Helps QA team to test the components in the dev sandbox
  lightweight - just a component - doesn't include extra dependencies or configuration
  Hope this has given you a bit of inspiration to take this concept and apply it to a similar problem you're tackling in your everyday workflow or on your side projects
</Notes>
---
# 🙏🏻
# Thank you!
